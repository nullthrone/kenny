...
| `local_accounts` | Accounts on the PC (local **and** Microsoft) plus the machine password policy — also the inventory for the `account_*` governance tools | an enabled admin with `password_required` false **and** no password ever set → **crit**; built-in Administrator or Guest enabled → **warn**; an admin that also carries denied logon rights → **warn** (one of the two settings is stale) (see [`account-governance.md`](account-governance.md)) |
| `logon_failures` | Failed sign-ins per account over 24 h, split by interactive / network / remote | ≥ 15 failures against a single account **or** ≥ 15 against usernames that do not exist here → **warn**. Never **crit** — a failed sign-in is not by itself a compromised machine |
