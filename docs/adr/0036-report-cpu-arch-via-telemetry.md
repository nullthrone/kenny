# 0036. Report CPU arch via telemetry; pin arch for fresh Linux installs
...
ADR-0034/#139's `register.meta.arch` field, so it never reported its arch on any
...
- Neutral, revises ADR-0034's consequence "no wire-contract change, no `PROTOCOL_VERSION`
...
- Builds on ADR-0034 (Linux distribution + self-update shape) and the #139 fix
