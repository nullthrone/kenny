# 0044. Scheduled update detection with a pinned, operator-approved rollout
...
agent binary on both Windows and Linux (ADR-0013/0038). Today it only runs when an operator
...
This has to be reconciled with two standing project decisions. ADR-0019 established a
project-wide confirm-gate: state-changing actions always pause for explicit operator
confirmation, never fire on their own. ADR-0038 went further and *explicitly considered and
...
- **B. "Track latest" auto-rollout.** A campaign is just an on/off switch scoped to an agent
  ...
  autonomous auto-update ADR-0038 already rejected, just with an extra checkbox in front of
  it.
- **C. Docker-socket sidecar (e.g. Watchtower scoped to `kenny-server`, or a small
  ...
  with no new privilege. Worth revisiting, additively, the same way ADR-0038 left autonomous
...
- **Detection** is a new background loop (`update_manager.py`), started from the app
  ...
  runtime-settings layer (`KENNY_UPDATE_CHECK_INTERVAL_SECS`, ADR-0036; `0` disables it, a
...
- **A rollout is a pinned campaign, not a live "track latest" toggle.** Approving a campaign
  ...
  *after* approval is a new, separately-approvable candidate. This is the load-bearing
  distinction from Option B and the reason this design does not collapse into ADR-0038's
...
- **Campaigns expire and can be revoked.** A campaign auto-completes once every known agent
  ...
  cannot be recalled, the same kind of can't-undo boundary ADR-0043 already named for
...
- **A per-agent attempt budget prevents error-loops.** `agent_update` is a mutating tool
  ...
  response (ADR-0039) is treated as "retry later," not counted against the budget, since it
...
- Good, because the architect's ask is met on both axes — a configurable interval and
  rollout to agents as they reconnect — without silently reversing ADR-0019's confirm-gate or
  reopening the autonomous auto-update ADR-0038 deliberately closed.
...
- Bad, because a held agent (attempt budget exhausted) needs an operator to notice and act —
  no channel wiring to push-alerting (ADR-0029) is built in this iteration; the dashboard
  state is the only signal for now.

## More Information

- Builds on: ADR-0013 (agent self-update mechanism), ADR-0015 (agent binary auto-fetch and
  version resolution), ADR-0036 (runtime settings — interval/GHCR-ref knobs), ADR-0038
  ...
  ADR-0039 (anti-cheat pause — source of the retry-without-penalty requirement), ADR-0043
  (backup/restore — precedent for naming a new trust boundary and for a can't-undo
  in-flight-operation caveat), ADR-0019 (confirm-gate principle), ADR-0010 (GHCR
  containerization).
...
- Deferred, additive follow-up (not built now): an optional off-by-default docker-socket
  ...
  (ADR-0029) wiring for held agents and failed detection passes.
