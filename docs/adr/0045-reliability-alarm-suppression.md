# 0045. Operator-managed reliability alarm suppression
...
only the two dashboard paths run the ADR-0028 LLM categorization that carries a `severity`
...
- **Teach the LLM classifier about this pattern (extend the ADR-0028 prompt, or a
  hand-maintained known-benign table).** Rejected: ADR-0028 exists precisely because the
...
- **A new setting in the ADR-0036 `Settings` catalog.** Rejected: settings are scalar
  key/value knobs; this needs a scoped, growable list of rules with per-row provenance
  (who, when, why), not a single value.
- **Stamp suppression at telemetry-insert time, like `web_activity.flagged` (ADR-0026).**
...
single-process/single-event-loop argument ADR-0036 makes for `Settings`): loaded at
startup, replaced on every write, matched with a synchronous, lock-free dict lookup —
`(source, event_id)` matching needs no LLM and no API key, so unlike the ADR-0028
...
annotations, exactly like `category`/`severity`/`suspected_cause` (ADR-0028) and
`web_activity.flagged` (ADR-0026) — the agent never sends or sees them, and
...
- Good, because the `TelemetryStore.annotate` seam is a single, well-tested integration
  ...
  ADR-0028 LLM annotation (different fields, no conflict, independently degradable).
- Good, because the Windows Reliability Index overlay and the "an `unknown` severity is
  never silently benign" property from ADR-0028 are both preserved unconditionally —
...
- [ADR-0028](0028-llm-categorization-of-reliability-events.md) — the `category`/`severity`/
  `suspected_cause` read-path annotation this decision sits alongside.
- [ADR-0026](0026-parental-controls-web-activity-and-webfilter.md) — the closest prior art
...
- [ADR-0021](0021-shared-policy-catalog-operator-rules-and-server-mirror.md) — the
  operator-append-only rule table + in-memory mirror pattern this reuses
  (`PolicyStore`/`PolicyEngine`).
- [ADR-0036](0036-runtime-settings-in-the-dashboard.md) — the "single process, one event
