# 0028. Security-inventory and resilience telemetry sections
...
administrator) — and is any backup mechanism alive? With the diff engine (`diffs.py`) in
...
Seven additive sections (plus `screen_time`, decided separately in ADR-0029) enter the
...
- `browser_extensions` reads extension manifests from the same per-user profile
  locations `web_activity` already walks. **Privacy stance inherited from ADR-0024:**
...
warn, heavy reference loss ⇒ crit), plus the `diffs.py` specs that notify on new
...
- Related: ADR-0007 (stateless collectors, section status/summary), ADR-0024 (privacy
  stance, profile enumeration), `diffs.py` (the diff engine consuming these sections),
  ADR-0029 (`screen_time`, the eighth v0.10 section).
