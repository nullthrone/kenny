# 0026. LLM categorization of reliability events
...
- The raw category label is a **server-side annotation**, not part of the wire
  contract — exactly like the `web_activity.flagged` annotation (ADR-0024). The
...
- **Bad / accepted.** Categories are non-deterministic across model versions
  ...
  already holds, and less sensitive than the `web_activity` data ADR-0024 already
