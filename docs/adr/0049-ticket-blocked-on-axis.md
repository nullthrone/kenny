# 0049. The ticket lifecycle splits into two axes: state and blocked-on
...
ADR-0046 made the ticket a first-class entity with a state machine, and the state machine
...
auto-ticket rules (`ticket_rules.py`) — and `kenny_server/webui/index.html` fetches it once instead
...
(the same idiom `UpdateStore._migrate` uses for its `channel` column, ADR-0048).
...
`from_state`/`to_state` keeps its original nine-state-era value. ADR-0046 makes the trail
...
- Amends [ADR-0046](0046-ticket-as-entity-chat-thread-as-binding.md): the lifecycle it
...
- Answers the deferral recorded in `ticket_rules.py`'s "deduplicating against an already-open ticket ... is a real idea,
...
- Patterned on [ADR-0041](0041-reliability-alarm-suppression.md) and
  [ADR-0048](0048-second-release-channel-dev-prereleases.md) for the migration idiom
