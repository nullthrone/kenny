# 0044. Delegated identity from a chat platform, with no parallel authorization

- Status: accepted
- Date: 2026-08-01
- Amends: [ADR-0023](0023-untrusted-agent-data-in-chat-context.md)
...
A second, related problem comes with the surface. ADR-0033 gated the dashboard copilot
...
1. the target host is fixed when the case is opened and nothing in the conversation can
   change it — the ADR-0038 control, applied twice (the selection tool is withheld from
   the surface, and a target argument coming back from the model is discarded, not
   adopted);
2. a per-user capability allowlist narrows the reachable tool set (ADR-0047);
3. the tier gate stops anything consequential and routes it to an operator (ADR-0045);
4. a consent gate covers tools that touch someone's privacy, independently of
   authorization (ADR-0047).
...
**The conversation itself is untrusted input, and multi-party.** ADR-0023 established that
...
- Bad, because a hostile message can still steer the model's choice among the tools the
  caller already has, exactly as ADR-0023 concedes for agent data. The deterministic gates,
...
- Extends [ADR-0023](0023-untrusted-agent-data-in-chat-context.md) from agent-supplied data
  to a multi-party conversation that a third party relays.
- Builds on [ADR-0033](0033-multi-user-authentication.md) (the `Principal`, roles, host
  scope) and [ADR-0038](0038-explicit-per-call-agent-targeting.md) (explicit, frozen
  targeting). Companion records: [ADR-0045](0045-tiered-tool-classification.md) (the tier
  gate), [ADR-0046](0046-ticket-as-entity-chat-thread-as-binding.md) (the case record),
  [ADR-0047](0047-capability-profiles.md) (the allowlist and the consent axis).
- The wire contract is untouched: this surface sits entirely above the tunnel, so
  ...
  posture as [ADR-0027](0027-push-alerting-ntfy-webhook-and-weekly-digest.md).
