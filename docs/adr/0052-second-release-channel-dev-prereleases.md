# 0052. Second release channel: `dev` prereleases via the GitHub `prerelease` flag
...
(`server_release.py`, ADR-0044). Nothing in the workflow, the wire contract, or the server
...
stays on stable. This has to compose with two decisions already on the books: ADR-0038's
Option C (autonomous agent-side auto-update, rejected) and ADR-0044 (a rollout is a pinned,
...
- **A. GitHub's `prerelease` flag drives the channel, one repo/one workflow family.**
  ...
  approved dev campaign can target — reusing ADR-0044's pinned-campaign mechanism verbatim,
...
- **C. Rolling `dev` tag, assets overwritten each push.** Keeps the release list short, but
  ...
  campaign pinning (ADR-0044) exists specifically so an approval names one concrete artifact.
...
- **Channel is a per-agent, server-held desired state**, mirroring ADR-0040's arch pattern
  ...
  operator-editable in the dashboard) — the same soll/ist split ADR-0044 already uses for
...
- **The rollout mechanism is unchanged.** A dev candidate is detected by the same
  `agent_release.py`/`server_release.py` polling ADR-0044 already runs, just pointed at
  ...
  (ADR-0038's rejected-autonomous-update reasoning applies unchanged) — so dev artifacts pass
...
- Good: does not reopen ADR-0038's rejected autonomous auto-update or ADR-0044's rejected
...
- Builds on ADR-0012 (prebuilt binary distribution), ADR-0015 (tag leads the version,
  `releases/latest` resolution), ADR-0038 (Linux distribution + self-update — source of the
  rejected autonomous-auto-update option this decision must not reintroduce), ADR-0040 (the
  register+telemetry dual-reporting pattern this reuses for `channel`), ADR-0044 (pinned,
