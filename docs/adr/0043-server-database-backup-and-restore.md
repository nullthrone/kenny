# 0043. Server database backup/restore with pluggable local + remote destinations
...
(ADR-0007, ADR-0027) on the `/data` volume (ADR-0010). The operator syncs that file
...
`op`/`su` role gating used for Settings (ADR-0037).
...
- Related: ADR-0007 (SQLite storage rationale this backs up), ADR-0010 (`/data` volume and
  the container restart policy the boot-time restore relies on), ADR-0027 (the multi-store
  single-file pattern — chat history, with embedded screenshots, is typically the largest
  and most sensitive part of what a backup carries), ADR-0036 (the runtime-settings pattern
  used for the interval/retention knobs), ADR-0037 (the superuser role gate reused for the
  Backup page).
- **Disambiguation:** this ADR is unrelated to ADR-0031's `backup_status` telemetry
...
- Follow-up (not built in this phase): a "backup overdue" / "last backup push failed"
  alert over the existing push-alerting channel (ADR-0029) once there is a natural signal
