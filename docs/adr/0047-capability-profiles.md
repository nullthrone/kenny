# 0047. Capability profiles: named per-user tool allowlists that only narrow

- Status: accepted
- Date: 2026-08-01
- Amends: [ADR-0033](0033-multi-user-authentication.md)

## Context and Problem Statement

ADR-0033 gave kenny two authorization axes: a role (how much of the fleet you govern) and a
...
Putting a household member in front of the tool loop (ADR-0044) makes the question
...
third axis beside role and host scope.** The role hierarchy from ADR-0033 is left exactly
...
- Good, because the ADR-0033 role hierarchy stays intact and keeps its meaning; capability
...
- Amends [ADR-0033](0033-multi-user-authentication.md) by adding a third axis to its
  role + host-scope model; nothing in that record is reversed.
- Depends on [ADR-0045](0045-tiered-tool-classification.md) for the tier term of the
  intersection, and is one of the four controls required by
  [ADR-0044](0044-delegated-identity-from-a-chat-platform.md).
- Related: [ADR-0024](0024-parental-controls-web-activity-and-webfilter.md) and
