# 0026. Parental controls: web-activity observability and on-demand web filtering
...
3. **The existing safety architecture must absorb this, not be worked around.** The local
   ...
   ([ADR-0020](0020-agent-side-deterministic-tool-guard.md)) all already gate *mutating*
...
- **List model — layered, per host.** A built-in **seed** of well-known adult domains ships
  ...
  equal-or-more-specific suffix. This mirrors the ADR-0021 split of a shared catalog vs.
...
- Good: the feature reuses the telemetry pipeline, the health-rule pattern, the ADR-0021 list
...
- Privacy stance: only **domains** cross the wire and are stored — never full URLs, page
  ...
  ([ADR-0024](0024-untrusted-agent-data-in-chat-context.md)).
...
- Related: [ADR-0007](0007-telemetry-push-model-and-sqlite-storage.md) (telemetry push + store),
  ...
  [ADR-0021](0021-shared-policy-catalog-operator-rules-and-server-mirror.md) (layered list model),
  [ADR-0024](0024-untrusted-agent-data-in-chat-context.md) (untrusted agent data).
