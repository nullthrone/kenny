# 0034. Linux agent distribution: convenience install script + server-triggered self-update
...
ADR-0031 decided first-class Linux agent support and implemented Phase 1 (the agent registers,
...
The Windows onboarding shape (ADR-0030) is a downloaded ZIP whose `setup.bat` self-elevates via
...
- Implements ADR-0031 Phase 4. Builds on ADR-0012 (prebuilt binary + config injection),
  ADR-0013 (server-triggered self-update), ADR-0022 (per-agent Ed25519 enrollment), and
  ADR-0030 (self-elevating bootstrap installer — the Windows onboarding shape this parallels).
