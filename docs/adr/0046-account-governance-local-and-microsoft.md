# 0046. Account governance across local and Microsoft accounts
...
by a local administrator: the hosts-file web filter (ADR-0026), the deny-rule guard's
...
ADR-0026 and ADR-0032 forbid per-user attribution, and both are enforced by tests:
...
- **Wire key: SID / qualified name (`MicrosoftAccount\…`) / SAM name (chosen).** The
  SID is unambiguous but reverses the ADR-0026 no-SIDs-on-the-wire invariant for no
...
- Good, because the strongest available control — removing local administrator rights —
  becomes a first-class, audited, confirm-gated action, which is what makes ADR-0026's
...
- **Bad, because the kill switch outranks governance.** All seven tools are mutating,
  ...
  state persists. This is ADR-0026's stance unchanged — **monitoring is the guarantee,
...
- Related: [ADR-0011](0011-local-remote-control-kill-switch.md) (kill switch),
  ...
  [ADR-0026](0026-parental-controls-web-activity-and-webfilter.md) (the stance this
  refines, and the control this one makes stick),
  [ADR-0031](0031-security-and-resilience-telemetry-sections.md) (`local_accounts`
  origin), [ADR-0032](0032-screen-time-aggregated-session-minutes.md) (aggregation rule
  left intact), [ADR-0037](0037-multi-user-authentication.md) (operator roles),
  [ADR-0042](0042-explicit-per-call-agent-targeting.md) (`agent_id` routing).
