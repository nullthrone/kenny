# 0043. Account governance on Linux
...
[ADR-0042](0042-account-governance-local-and-microsoft.md) gave kenny the ability to steer
...
The fleet is not Windows-only. [ADR-0031](0031-linux-agent-support.md) made Linux a
first-class agent target, [ADR-0034](0034-linux-agent-distribution-convenience-script.md)
...
control hold (ADR-0042's own argument), and on a Linux home server — where sudo is the whole
...
would become three, because ADR-0042 already had to solve this once for local-vs-Microsoft
...
- **A second `linux_account_*` tool family.** Rejected for the same reason ADR-0042 rejected
...
- **Widen the OS scope and carry every Linux asymmetry in the existing per-account
  `unsupported` map (chosen).** ADR-0042 built that map for exactly this: "the
...
- **The admin group is resolved, not assumed** — the first of `sudo`, `wheel`, `admin` that
  ...
  deterministic analogue of ADR-0042's well-known-SID trick: distro-proof the way the SID is
  locale-proof.
...
- **The privacy line does not move again.** ADR-0042 moved it once: identity is named and
...
- **The Linux arm shells out with `Command::new` directly**, not through `shell_exec`. The
  `posix` deny-rule catalog (ADR-0019/0020) therefore does not apply to it. That is correct
...
- Bad, because the wire vocabulary stays Windows-flavoured: `remote_interactive` means SSH on
  Linux, and `local_accounts` names a section that includes Microsoft accounts. ADR-0031
...
- Bad, because Linux fragmentation is real and this cannot be fully tested in CI. The sshd
  ...
  on.
- Bad, because the kill switch still outranks governance (ADR-0042's stance, unchanged): all
...
- Bad, because `shell_exec` bypasses the whole layer on Linux exactly as `powershell_exec`
  ...
  reason ADR-0042 did not address it.
...
- Explicitly **out of scope**: directory-backed identities (SSSD/LDAP/AD-joined hosts — they
  would arrive as a new `kind` with a large `unsupported` map, exactly as ADR-0042 predicted
...
- Related: [ADR-0042](0042-account-governance-local-and-microsoft.md) (the record this
  extends — its Windows rationale stands unchanged),
  [ADR-0031](0031-linux-agent-support.md) (first-class Linux, additive growth — this is its
  ...
  [ADR-0020](0020-shared-policy-catalog-operator-rules-and-server-mirror.md) (the deny
  ...
  [ADR-0024](0024-parental-controls-web-activity-and-webfilter.md) /
  [ADR-0029](0029-screen-time-aggregated-session-minutes.md) (the privacy line, unmoved),
  [ADR-0033](0033-multi-user-authentication.md) (the operator role gate, unchanged).
