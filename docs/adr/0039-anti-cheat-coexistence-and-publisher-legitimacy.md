# 0039. Anti-cheat coexistence and publisher legitimacy
...
- **PE identity (`kenny-agent/build.rs`, via `winresource`).** The Windows binary now carries
  ...
  and the exe icon. `embed-manifest` keeps owning the `asInvoker` manifest (ADR-0033);
...
- **Code signing is prepared, not enabled here.** The Authenticode step in `release.yml` is
  ...
  unmodified (ADR-0033/0012) so a signature survives distribution. Turning it on needs an
...
- Relates to ADR-0011 (kill-switch `disabled` — the adjacent "online but refusing" code),
  ADR-0018 (`screen_capture` via the tray — the suspended action), ADR-0020/0021 (the
  dispatch gates this sits beside), ADR-0031/0032 (the process/port/screen-time telemetry it
  relaxes), and ADR-0033/0012 (signature-preserving distribution).
...
- Implementation: `kenny-agent/src/coexist.rs` (new), `dispatch.rs`, `tunnel.rs`,
  ...
  `KENNY_COEXIST_TELEMETRY_INTERVAL_SECS` (env, per ADR-0036).
