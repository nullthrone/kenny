# 0037. OAuth 2.1 authorization server for the MCP connector

- Status: accepted
- Date: 2026-07-21
- Amends: [ADR-0033](0033-multi-user-authentication.md), [ADR-0008](0008-operator-authentication.md), [ADR-0014](0014-auth-hardening.md)
...
(PAT, ADR-0033) in the dashboard and paste it into the client as an
...
identity model ADR-0033 deliberately chose (it rejected delegating identity to an external
...
- *External IdP* — rejected, same reasoning as ADR-0033: it moves the household's identity
...
- Good: no wire-contract impact. This is server-only and does not touch the agent⇄server
  ...
  handshake, exactly as ADR-0033 was.
...
- Amends ADR-0033 (adds OAuth as a third credential type alongside PAT and session,
