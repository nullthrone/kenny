# 0031. First-class Linux agent support
...
installs as a Windows service (ADR-0013/0030), speaks the outbound WSS wire contract
...
Ed25519 auth (ADR-0022), framing, config, the kill switch (ADR-0011), and the safety guard
(ADR-0019) are ungated and Linux-CI-tested. `os_family()` (`kenny-agent/src/util.rs`)
...
- **Mutual Ed25519 auth** (`src/keys.rs`, `src/tunnel.rs`, ADR-0022): keygen/sign/verify are
...
- **Dispatch, kill switch, safety guard** (`src/dispatch.rs`, `src/control.rs`,
  `src/policy.rs`, ADR-0011/0019): portable; `os_family()` already reports `"linux"`.
...
- **The named-pipe IPC + session-0 model** (`src/screencap_ipc.rs`,
  `src/session_launch_ipc.rs`, `src/tray.rs`; ADR-0018/0021): a session-0 service launching
...
- **`powershell_exec`** keeps its name but on Linux the "PowerShell" is a shell (`sh -c`,
  ...
  (`docs/policy/deny_rules.json`, ADR-0019/0020) match against *shell* commands on Linux — a
...
ADR-0011 (kill switch), ADR-0013 / ADR-0030 (Windows service + bootstrap installer — the
lifecycle this replaces on Linux), ADR-0018 (screenshots via the tray/session-0 helper),
ADR-0019 / ADR-0020 (safety guard + policy catalog — the PowerShell-vs-shell design point),
ADR-0021 (Quick Assist concierge — Windows-only), ADR-0022 (mutual Ed25519 auth — portable),
ADR-0024 (webfilter — adaptable core), ADR-0028 / ADR-0029 (telemetry sections). The wire
