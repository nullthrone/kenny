# 0038. Explicit per-call agent targeting for forwarded MCP capability tools
...
ADR-0033 made that slot per-caller (`registry._active_by_key`, keyed by
...
- **MCP path (`kenny_server/tools.py`, the reported bug).** `agent_id` is **required**. The
  ...
  for discovery), and the keyed-slot plumbing from ADR-0033 stays in the registry, but it no
...
- **Server-hosted dashboard chat (`kenny_server/chat.py`).** Each `ChatSession` is already a
  ...
  ADR-0033 at all, since it never went through a key). `run_capability` now takes the resolved
...
- Supersedes ADR-0033's per-principal active-agent isolation as the routing mechanism for MCP
  forwarding (ADR-0033's keyed slot and `select_agent` remain, now advisory/back-compat only).
