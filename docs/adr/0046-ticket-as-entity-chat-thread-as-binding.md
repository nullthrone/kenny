# 0046. The ticket is the entity; the chat thread is a binding

- Status: accepted
- Date: 2026-08-01
- Amends: [ADR-0025](0025-persistent-chat-history.md)

## Context and Problem Statement

The chat surface from ADR-0044 needs somewhere to keep a support case: what was asked, what
...
one that asked. ADR-0025 deliberately kept all transient turn state process-local — pending
...
**This reverses ADR-0025's "pending state is never persisted" — for this surface only.**
A case's resume state is durable; dashboard chat sessions stay transient, and ADR-0025's
...
- Neutral: ADR-0025 stands as written for the dashboard. This record carves out an
  exception for a surface whose humans are not watching, it does not reverse the decision.

## More Information

- Amends [ADR-0025](0025-persistent-chat-history.md) (the process-local-transient-state
...
- Related: [ADR-0017](0017-observability-logging-and-event-store.md) — the per-case trail is
  ...
  [ADR-0027](0027-push-alerting-ntfy-webhook-and-weekly-digest.md) — alerts as a case
  origin; [ADR-0044](0044-delegated-identity-from-a-chat-platform.md) — the surface this
  serves.
