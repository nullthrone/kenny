# 0024. Treat agent-supplied data as untrusted in the chat tool-use loop
...
classified mutating on the agent (`control.rs::is_mutating`, ADR-0022) but were
...
deterministic safety controls (agent-side tool guard ADR-0020/0021; the operator
...
- Related: ADR-0009 (server-hosted chat + confirm-gate), ADR-0018 (screen capture is
  read-only), ADR-0020/0021 (deterministic agent-side tool guard + server mirror),
  ADR-0022 (remote-help concierge / `remotehelp_*`).
