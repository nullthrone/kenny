# 0050. The ticket is its own chat surface

- Status: accepted
- Date: 2026-08-04
- Amends: [ADR-0045](0045-tiered-tool-classification.md), [ADR-0046](0046-ticket-as-entity-chat-thread-as-binding.md)
- Touches: [ADR-0044](0044-delegated-identity-from-a-chat-platform.md), [ADR-0049](0049-ticket-blocked-on-axis.md)

> Numbered 0055, not 0054: this record and
> [ADR-0049](0049-ticket-blocked-on-axis.md) (the lifecycle's two-axis split) were developed
...
> code, ADR-0049 changed what it says.
...
1. **Kenny's own replies leave no durable wording anywhere.** A `message` trail row for a
   ...
   resume-only transcript that ADR-0046 already prunes 30 days after close. Reload the
...
autonomously (ADR-0045), whose identity and controls make that defensible (ADR-0044), and
what does a ticket keep a record of (ADR-0046)? A second transport does not get to reopen
...
### Amends ADR-0046: the trail now carries verbatim text — for curated work, not for a private conversation
...
This is not a reversion to ADR-0046's rejected "thread-as-record" option. ADR-0046 already
...
[ADR-0025](0025-persistent-chat-history.md) already accepted when it let the copilot's chat
...
### Amends ADR-0045: a second surface now holds the ticket's tiered, autonomous gate

ADR-0045 separated a tool's tier (what kind of change it is) from a surface's policy (what
...
— the object ADR-0044's four controls were written for, reachable now from a second
transport rather than only from Discord.

### Touches ADR-0044: same controls, no delegation on this path

ADR-0044's four controls were framed for a platform kenny does not own, where the danger is
...
a loosening of ADR-0044 — it is the same gate, reachable from a transport that never needed
...
([ADR-0049](0049-ticket-blocked-on-axis.md)'s blocked-on axis), and an operator is by
...
[ADR-0027](0027-push-alerting-ntfy-webhook-and-weekly-digest.md) and
[ADR-0044](0044-delegated-identity-from-a-chat-platform.md) record for their own additions.
...
- Neutral, because none of ADR-0044's, ADR-0045's, or ADR-0046's underlying guarantees moved
...
- Builds on [ADR-0009](0009-server-hosted-claude-chat.md) (the tool-use loop and its
  confirm-gate) and [ADR-0038](0038-explicit-per-call-agent-targeting.md) (the frozen-target
  guarantee this record does not touch: a ticket's `agent_id` remains unmovable by anything
  said in either surface's chat).
- Amends [ADR-0045](0045-tiered-tool-classification.md) (a second surface now qualifies for
  tiered autonomy) and [ADR-0046](0046-ticket-as-entity-chat-thread-as-binding.md) (the
  trail's `message` kind may now carry verbatim text for curated work).
- Touches [ADR-0044](0044-delegated-identity-from-a-chat-platform.md): three of its four
