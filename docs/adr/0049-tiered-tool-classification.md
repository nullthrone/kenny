# 0049. Tiered tool classification: the tier belongs to the tool, the gate to the surface
...
A surface where a non-operator drives tools (ADR-0048) breaks the assumption. Flushing a
...
ADR-0024 exists to prevent. Loosening the dashboard later is therefore its own visible
...
- **Parity with the agent.** The agent enforces its own deterministic notion of what
  mutates (ADR-0020/0021). The server's tiers must never call something read-only that the
  agent considers mutating; a test holds a literal copy of the agent's list and fails the
  build if a mutating tool is unclassified or mis-tiered. This is ADR-0024's "one source of
...
- Refines [ADR-0009](0009-server-hosted-claude-chat.md)'s binary classification and
  confirm-gate; reinforces [ADR-0024](0024-untrusted-agent-data-in-chat-context.md), whose
...
- Depends on the deterministic agent-side guard,
  ...
  [ADR-0021](0021-shared-policy-catalog-operator-rules-and-server-mirror.md), as the
  authority the server's tiers are checked against.
- Consumed by [ADR-0048](0048-delegated-identity-from-a-chat-platform.md) as one of the four
