# 0021. Shared policy catalog, operator deny rules, and a server-side mirror
...
ADR-0020 introduced a deterministic, always-on safety guard on the agent that refuses
...
- Bad / trade-offs: the agent build now embeds a file from `docs/`; the server build must
  ...
  ADR-0020, this is defense-in-depth below auth + confirm-gate + kill-switch, not a sandbox.
...
- Related: [ADR-0020](0020-agent-side-deterministic-tool-guard.md) (the guard),
