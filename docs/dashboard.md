...
<figure markdown>
  ...
  <figcaption>The per-agent drill-down: action buttons, the health-trend sparkline, every telemetry section as a tile, the changes &amp; forecast panel, and the last screenshot.</figcaption>
</figure>
...
- **Changes & forecast · 24 h** — inventory changes vs yesterday's snapshot (added/removed
  software, services, accounts, …) plus disk-fill and battery forecasts (e.g.
  "*C: ~16 d until full (96 % now, +0.25 %/day)*"). See [Alerting & forecasts](alerting.md).
